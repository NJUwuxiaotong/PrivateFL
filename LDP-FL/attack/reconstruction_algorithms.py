...
class GradientReconstructor():
    ...
    def __init__(self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG,
                 num_images=1):
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32),
                    dryrun=False, eval=True, tol=None):
        """Reconstruct image from gradient.
            input_data: gradient, type -> list
        """
        ...
        if labels is None:
            if self.num_images == 1 and self.iDLG:
                # iDLG trick:
                last_weight_min = \
                    torch.argmin(torch.sum(input_data[-2], dim=-1), dim=-1)
                labels = \
        ...
        try:
            for trial in range(self.config['restarts']):
                ...
                # Finalize
                # compute the score based on the input data and the
                # predicted labels
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            labels = torch.randn(output_test.shape[1])\
                .to(**self.setup).requires_grad_(True)
        ...
        try:
            # iterate to reconstruct the raw image from gradients
            ...
            pass
        # tensor.detach(): construct a new view that needs no gradient
...
class FedAvgReconstructor(GradientReconstructor):
    ...
    def _gradient_closure(self, optimizer, x_trial, input_parameters, labels):
        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            parameters = loss_steps(self.model, x_trial, labels,
...
def loss_steps(model, inputs, labels, loss_fn=torch.nn.CrossEntropyLoss(),
               lr=1e-4, local_steps=4, use_updates=True, batch_size=0):
    ...
    for i in range(local_steps):
        ...
        patched_model.parameters = OrderedDict(
            (name, param - lr * grad_part) for ((name, param), grad_part)
...
def reconstruction_costs(gradients, input_gradient, cost_fn='l2',
    ...
    if isinstance(indices, list):
        ...
        _, indices = torch.topk(torch.stack([p.norm() for p in input_gradient],
                                            dim=0), 4)
    elif indices == 'top10':
        _, indices = torch.topk(torch.stack([p.norm() for p in input_gradient],
                                            dim=0), 10)
    ...
    if weights == 'linear':
        weights = torch.arange(
            len(input_gradient), 0, -1,
            dtype=ex.dtype, device=ex.device) / len(input_gradient)
    elif weights == 'exp':
        weights = torch.arange(len(input_gradient), 0, -1,
                               dtype=ex.dtype, device=ex.device)
